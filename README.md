...
5. **나만의 쿼카 (My Page) 탭 (New!)**: 내가 좋아요를 누른 쿼카 사진들을 한곳에 모아보고, 원할 때 다시 취소(Unlike)할 수 있는 웹 갤러리
6. **부드러운 UX 경험 (New!)**: 데이터 호출 간 로딩 스피너, 직관적인 토스트(Toast) 알림 및 업로드 전 이미지 미리보기 적용
7. **반응형 웹 디자인**: 데스크탑, 태블릿, 모바일 환경 완벽 지원
...
- [x] **페이지 확장 및 UI 완성**: Pricing, Business 메뉴 추가 및 폴라로이드 감성 CSS 적용
- [x] **소셜 로그인 연동**: 구글, 깃허브 계정을 통한 간편 1초 로그인 도입 완료
- [x] **성능 및 UX 최적화**: API 데이터 통신 로딩 스피너 추가 및 간격 디자인 조정
- [x] **실용 기능 고도화**: 토스트(Toast) 알림 UI, 도감 '좋아요 취소' 기능, 업로드 파일 미리보기 및 검증 로직 추가

### 🔜 앞으로의 계획 (Todo)

- [ ] **다크 모드 (Dark Mode) 지원**: 밤에도 눈부심 없이 쿼카를 볼 수 있는 테마 추가
- [ ] **쿼카 월드컵 기능**: 랜덤으로 나오는 2마리의 쿼카 중 더 귀여운 쿼카를 고르는 미니게임 추가
- [ ] **Pricing 페이지 고도화**: 월간/연간 결제 토글 스위치 및 가상 결제(Checkout) 모달 팝업 추가
- [ ] **Business 페이지 고도화**: 도입 문의 폼 유효성 검사(Validation) 및 아코디언(Accordion) FAQ UI 적용
- [ ] **Documentation 페이지 고도화**: 코드 스니펫 원클릭 복사(Copy) 버튼 및 대화형 API 테스트 뷰어 도입
